...
static void	insert_que(n *arr_min, n *arr_max, int *min, int *max)
{
	...
	printf("arr_min[%d] value : %d, key : %d\n", idx_min, arr_min[idx_min].value, arr_min[idx_min].key);
...
static void	delete_min(n *arr_min, int *min)
{
	...
	printf("delete min ! arr_min[1].value : %d, arr_min[1].key : %d\n",  arr_min[1].value, arr_min[1].key);
...
static void	delete_max(n *arr_max, int *max)
{
	...
	int	parent = 1, child = 2;
	int	temp_value = arr_max[*max].value, temp_key = arr_max[*max].key, idx;
	...
	idx = 0;
	while (++idx <= *max)
		printf("arr_max[%d] = %d ::", idx, arr_max[idx].value);
	printf("\n");

	printf("!!!!arr_max[1] : %d, key : %d\n", arr_max[1].value, arr_max[1].key);
	...
	while (test_num-- > 0)
	{
		...
		while (data_num-- > 0)
		{
			...
			if (temp == 'I')
			{
				...
				idx = 0;
				while (++idx <= min)
					printf("arr_min[%d] = %d ::", idx, arr_min[idx].value);
				printf("\n");
				idx = 0;
				while (++idx <= max)
					printf("arr_max[%d] = %d ::", idx, arr_max[idx].value);
			...
			else
			{
				...
				{
					printf("arr_min value : %d, key : %d, data : %d\n", arr_min[1].value, arr_min[1].key, data[arr_min[1].key]);
					delete_min(arr_min, &min);
				}
				idx = 0;
				while (++idx <= min)
			...
			printf("data_num : %d, count : %d, min : %d, max : %d\n", data_num, count, min, max);
		}
		if (count == 0)
			printf("EMPTY\n");
		else
